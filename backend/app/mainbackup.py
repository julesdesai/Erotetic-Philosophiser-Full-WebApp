...
class ConnectionManager:
    def __init__(self):
        ...
        self.graphs: Dict[str, QuestionGraph] = {}  # Store graphs by client_id
    ...
    async def initialize_session(self, client_id: str, initial_question: str):
        """Initialize graph and discussion bot for a new session"""
        ...
        # Create discussion bot using the same graph
...
async def chat_websocket_endpoint(websocket: WebSocket, client_id: str):
    try:
        ...
        # Wait for initial question from client
        initial_data = await websocket.receive_json()
        initial_question = initial_data.get('message', "What is knowledge?")
        
        # Initialize session with shared graph
        await manager.initialize_session(client_id, initial_question)
        await manager.connect_chat(websocket, client_id)
        
        # Send initial graph data for visualization
        graph_data = manager.get_graph_data(client_id)
        await websocket.send_json(graph_data)
        
        # Get the discussion bot instance
        ...
        while True:
            ...
            if data["type"] == "message":
                ...
                # Send updated graph data after each interaction
                graph_data = manager.get_graph_data(client_id)
                await websocket.send_json(graph_data)
